<?php if(login_check() >= 999): ?>
	<?php 
		...
		$search_results = $cl->Query($search_string,'search');
		...
		<div class='row'>
			...
			<?php else: ?>
				...
				<?php 
					...
					include $PATH['INC'].'query_search.php'; 
				?>
					<div class='col-xs-12 col-sm-8 col-md-6 margin-15'>
						<div class='col-xs-12'>
							<a href=<?='?view=admin_sw_details&swid='.$search_view_results['lic_sw']; ?>><?=$search_view_results['sw_name']; ?></a>
						</div>
						<div class='col-xs-12'>
							<small>
								<a href=<?='?view=admin_sw_details&swid='.$search_view_results['lic_sw']; ?>
								   title=<?=__ROOT__.'?view=admin_sw_details&swid='.$search_view_results['lic_sw'];?>>
									<?=limit_str(__ROOT__.'?view=admin_sw_details&swid='.$search_view_results['lic_sw'],50);?>
								</a>
							</small>
						</div>
						<div class='col-xs-6'>
							<span class="key">Assigned User:</span><a href=<?='.?view=user_details&username='.$search_view_results['lic_user_samaccount']; ?>><?=$search_view_results['lic_user']; ?></a>
						</div>
						<div class='col-xs-6'>
							<span class="key">Company:</span><?=$search_view_results['bu_name']; ?>
						</div>
						<div class='col-xs-12'>
