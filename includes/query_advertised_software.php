...
if (!isset($tab_type)){
	if(isset($_GET['type']) && urldecode($_GET['type']) != "All"){$type = urldecode($_GET['type']);}else{$type=0;}
} else {
	$type = $tab_type;
}
...
die();*/
	...
	//Get details of currently tapped beers and total
		if($filter == '' && $sortby == '')
			{
				$query = "SELECT 	sw_id,sw_mfg,mfg_name,sw_os_windows,sw_os_osx,sw_logo,sw_auto_approve,sw_advertise,
									sw_os_linux,sw_type,type_name,sw_name,sw_desc,mfg_logo,type_color,
									count(sw_name) AS 'total_license',count(nullif(lic_user_samaccount,'')) AS 'assigned_license',
									count(sw_name) - count(nullif(lic_user_samaccount,'')) AS 'available_license'
							FROM lms_sw
							LEFT JOIN lms_lic
							ON (lms_sw.sw_id = lms_lic.lic_Sw)
							LEFT JOIN lms_mfg
							ON (lms_sw.sw_mfg = lms_mfg.mfg_id)
							LEFT JOIN lms_type
							ON (lms_sw.sw_type = lms_type.type_id)
							WHERE sw_mfg !=0 AND sw_advertise = 1
              GROUP BY sw_name
		...
		else
		{
				$query = "SELECT 	sw_id,sw_mfg,mfg_name,sw_os_windows,sw_os_osx,sw_logo,sw_auto_approve,sw_advertise,
							...
							LEFT JOIN lms_type
							ON (lms_sw.sw_type = lms_type.type_id)
							WHERE sw_advertise = 1 AND sw_mfg !=0".$filter." GROUP BY sw_name".$sortby;
		}
