<?php
		$query = "SELECT 	lic_id,lic_sw,lic_lic_type,lic_key,lic_user,lic_bu,
							lic_type_name,bu_name,lic_user_samaccount,bu_id
					FROM lms_lic
					LEFT JOIN lms_lic_type
					ON (lms_lic.lic_lic_type = lms_lic_type.lic_type_id)
					LEFT JOIN lms_bu
					ON (lms_lic.lic_bu = lms_bu.bu_id)
					WHERE lic_sw=".$software_id." AND lic_bu=".$bu_id." ORDER BY lic_user,lic_id";
		...
		$query = "SELECT 	lic_id,lic_sw,lic_lic_type,lic_key,lic_user,lic_bu,
					...
					LEFT JOIN lms_bu
					ON (lms_lic.lic_bu = lms_bu.bu_id)
					WHERE lic_user !='' AND lic_sw=".$software_id." AND lic_bu=".$bu_id;
